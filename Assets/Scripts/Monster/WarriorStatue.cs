...
public class WarriorStatue : Monster
{
    ...
    public void Attack0()
    {
        if (CanAttackLogic())
        {
            ...
            StartCoroutine(SpawnEffect(0, _target.transform.position, 0.4f));
    ...
    public void Attack1()
    {
        if (CanAttackLogic())
        {
            ...
            _animator.SetTrigger("attack");
            StartCoroutine(SpawnFallingSword(swordCount, swordInterval));
    ...
    public IEnumerator SpawnEffect(int idx, Vector2 pos, float time)
    {
        yield return new WaitForSeconds(time);
        int a;
        if (Random.Range(0, 1f) < 0.5f) a = 1;
        else a = -1;
        var i = Instantiate(effects[idx], pos+new Vector2(a, 0), Quaternion.identity);
        i.GetComponentInChildren<EffectAttack>().SetInfo(
            _monsterInfo.atk * _monsterInfo.attackCoefficient[idx], 
            _monsterInfo.attackStunTime[idx],
            _monsterInfo.attackKnockback[idx],
            new Vector3(a, 1, 1));
    }

    public IEnumerator SpawnFallingSword(int count, float time)
    {
